...
async function refresh(){try{const d=await send({type:"health"});$("#status").textContent=d.lastError?`提示：${d.lastError}`:d.running?`● ${d.modelStatus || "等待聲音"}`:"● 已就緒";const x=d.diagnostics||{};$("#diagnostics").textContent=`音訊：${x.frames||0} 區塊\n音量：${Math.round((x.level||0)*1000)}\n模型：${x.ready?'就緒':'尚未就緒'}\n辨識：${x.decodeMs||0} ms\n翻譯：${x.translationMs||0} ms`;$("#start").disabled=d.running;$("#stop").disabled=!d.running}catch(e){$("#status").textContent=e.message}}
