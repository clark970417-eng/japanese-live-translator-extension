...
const defaults={japaneseColor:"#ffffff",chineseColor:"#ffffff",fontSize:22,outlineWidth:1,backgroundColor:"#000000",backgroundOpacity:60,position:6,holdSeconds:3,captionMode:'record'};
...
const controls=["japaneseColor","chineseColor","fontSize","outlineWidth","position","holdSeconds","captionMode","backgroundColor","backgroundOpacity"];
function values(){return Object.fromEntries(controls.map(id=>[id,["fontSize","outlineWidth","position","holdSeconds","backgroundOpacity"].includes(id)?Number($("#"+id).value):$("#"+id).value]))}
function paintLabels(){$("#backgroundOpacityValue").textContent=`${$("#backgroundOpacity").value}%`;$("#holdSecondsValue").textContent=`${$("#holdSeconds").value} 秒`;$("#fontSizeValue").textContent=`${$("#fontSize").value} px`;$("#outlineWidthValue").textContent=`${$("#outlineWidth").value} px`}
