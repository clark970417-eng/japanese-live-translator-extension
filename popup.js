...
const defaults={japaneseColor:"#ffffff",chineseColor:"#ffffff",fontSize:25,outlineWidth:2,position:6,holdSeconds:3};
...
const controls=["japaneseColor","chineseColor","fontSize","outlineWidth","position","holdSeconds"];
function values(){return Object.fromEntries(controls.map(id=>[id,["fontSize","outlineWidth","position","holdSeconds"].includes(id)?Number($("#"+id).value):$("#"+id).value]))}
function paintLabels(){$("#holdSecondsValue").textContent=`${$("#holdSeconds").value} 秒`;$("#fontSizeValue").textContent=`${$("#fontSize").value} px`;$("#outlineWidthValue").textContent=`${$("#outlineWidth").value} px`}
