...
const defaults={japaneseColor:"#ffffff",chineseColor:"#ffffff",fontSize:22,showOutline:true,outlineWidth:1,captionOpacity:100,backgroundColor:"#000000",backgroundOpacity:60,position:6,holdSeconds:3,captionMode:'realtime'};
...
const controls=["japaneseColor","chineseColor","fontSize","showOutline","outlineWidth","position","holdSeconds","captionMode","backgroundColor","backgroundOpacity","captionOpacity"];
...
function paintLabels(){const preview=$("#captionPreview"),outline=$("#showOutline").checked&&Number($("#outlineWidth").value)>0;preview.style.backgroundColor=$("#backgroundColor").value+Math.round(Number($("#backgroundOpacity").value)*2.55).toString(16).padStart(2,"0");$("#outlineWidth").disabled=!$("#showOutline").checked;preview.style.webkitTextStroke=outline?"0.4px #000":"0 transparent";preview.style.textShadow=outline?"0 0 2px #000":"none";preview.style.opacity=Number($("#captionOpacity").value)/100;preview.style.fontSize=Math.min(26,Number($("#fontSize").value))+"px";$("#previewJa").style.color=$("#japaneseColor").value;$("#previewZh").style.color=$("#chineseColor").value;$("#captionOpacityValue").textContent=`${$("#captionOpacity").value}%`;$("#backgroundOpacityValue").textContent=`${$("#backgroundOpacity").value}%`;$("#holdSecondsValue").textContent=`${$("#holdSeconds").value} 秒`;$("#fontSizeValue").textContent=`${$("#fontSize").value} px`;$("#outlineWidthValue").textContent=`${$("#outlineWidth").value} px`}
