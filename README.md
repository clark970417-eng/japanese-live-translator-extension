...
日中同字級，預設 22px 白字與 1px 黑邊，組間留半行；視窗背景透明，每行文字有 60% 黑色半透明底，可在設定調整底色與透明度（0% 為透明）；長句只在寬度不足時自然換行。按住字幕區拖曳，拉邊緣或角落縮放，位置與大小會保存。四組在完整模式下保留至新句替換；即時模式才使用秒數消失設定。縮小到無法容納內容時可捲動字幕區。
