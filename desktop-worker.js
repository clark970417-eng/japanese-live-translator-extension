...
export class DesktopWorker {
 ...
 postMessage(data){
  this.id=data.id;
  const fields={segment:data.segment,utterance:data.utterance,final:data.final,speechSeconds:data.speechSeconds,speechAt:data.speechAt,audioEndAt:data.audioEndAt};
  if(data.type==='decode'){
