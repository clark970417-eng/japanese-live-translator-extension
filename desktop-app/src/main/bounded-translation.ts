...
export async function boundedTranslation(
 ...
 const cancel = (): void => controller.abort(new Error('Translation cancelled'))
 ...
 try {
  ...
  if(controller.signal.aborted || result.stopReason === 'abort')throw new Error('Translation timed out')
