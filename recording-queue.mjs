...
export class RecordingQueue {
 constructor({save,translate,onChange=()=>{},onError=()=>{}}){this.save=save;this.translate=translate;this.onChange=onChange;this.onError=onError;this.entries=[];this.busy=false;this.writes=Promise.resolve();}
 ...
 async pump(){
  ...
  try{
   for(;;){
    ...
    try{entry.translated=await this.translate(entry.original);entry.state='done';delete entry.error;}
 ...
 rows(session){
  ...
  for(const e of this.entries){if(e.session!==session||e.state==='pending')continue;let row=groups.get(e.group);if(!row){row={id:e.group,original:'',translated:'',pending:0};groups.set(e.group,row);}row.original+=(row.original?' ':'')+e.original;row.translated+=(row.translated&&e.translated?' ':'')+e.translated;if(e.state!=='done')row.pending++;}
